...
class MultiSportsDataset(BaseDataset):
    """MultiSports dataset for spatio-temporal action detection.

    Based on official MultiSports annotation files, the dataset loads raw frames,
    ...
    def load_annotations(self):
        ...
        with open(self.ann_file, 'r') as fin:
            for line in fin:
                ...
                img_key = f'{video_id},{timestamp:d}'

                entity_box = np.array(list(map(float, line_split[2:6])))
                entity_id = int(line_split[7])
                timestamp_end = int(line_split[8]) + 1
                shot_info = (1, timestamp_end)
